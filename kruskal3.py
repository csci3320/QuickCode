...
for edge in edges:
  first = edge["a"]
  second = edge["b"]
  if first not in listOfNodes:
    listOfNodes.append(first)
  if second not in listOfNodes:
    listOfNodes.append(second)
...
trees = {}
for nodeIndex in range(len(listOfNodes)):
  node = listOfNodes[nodeIndex]
  trees[node] = nodeIndex
...
for edge in edges:
  ...
  firstNumber  = trees["Westgate"]
  foundDifference = False
  for pair in trees:
    if trees[pair] != firstNumber:
